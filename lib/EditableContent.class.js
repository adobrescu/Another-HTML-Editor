...
function EditableContent_surroundSelectedContentFragments(tagName, tagAttributes, select)
{
	...
	{
		var c=this.surroundContentFragment(tagName, tagAttributes,
...
function EditableContent_onUserSelectionChanged(evt)
{
	...
	try
	{
		...
		phrasingContent=new PhrasingContent(this);
		
		if(this.currentPhrasingContent && phrasingContent.isEqualPhrasingContent(this.currentPhrasingContent))
		{
			return;
		}
		if(this.currentPhrasingContent)
		{
			this.currentPhrasingContent.removeHighlight();
		}

		phrasingContent.highlight();
		this.currentPhrasingContent=phrasingContent;
		this.mutationHistory.setMutationSplitNode(phrasingContent.crtHighlightElements[0].marker, "addedNodes");
		return;
	}
	catch(err)
	{
