...
with(EditableContent)
{
	...
	prototype.logNode=HtmlEditor_logNode;
...
function HtmlEditor_logNode(node, msg)
{
	console.log((msg?msg+" ":"")+(node.nodeType==3?"Text: "+node.textContent:(node.nodeType==1?node.tagName+(node.id?" #"+node.id:""):node.nodeType)));
}
...
{	
	...
	{
		//this.logNode(callbackMethodArguments.delimiterNodes[numDelimiters-1].lastNode, "End tag: ");
