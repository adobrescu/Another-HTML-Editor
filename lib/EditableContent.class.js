...
function EditableContent(iframe, innerHTML, serializedHistory)//always an iframe, need edited doc CSS in their own space
{
	...
	this.documentContainer.addEventListener("keydown", new Function("evt", "EditableContent.prototype.___instances["+EditableContent.prototype.___numInstances+"].onKeyDown(evt)" ));
	/*
...
with(EditableContent)
{
	...
	prototype.scrollToNodes=EditableContent_scrollToNodes;
	prototype.scrollToSelection=EditableContent_scrollToSelection;
	...
	prototype.onKeyDown=EditableContent_onKeyDown;
	
	prototype.preWrapTextNode=EditableContent_preWrapTextNode;
	prototype.preWrap=EditableContent_preWrap;
}
...
function EditableContent_addSelection(startContainer, startOffset, endContainer, endOffset, isCustom)
{
	this.window.contentSelection.addRange({ "startContainer": startContainer, "startOffset": startOffset, "endContainer": endContainer, "endOffset": endOffset, "isCustom": isCustom });
...
function EditableContent_scrollToNodes(topNode, bottomNode)
{
	var scrollTop;
	var bcr;	
	...
	if(topNode.isText())
	{
		var range=this.documentContainer.ownerDocument.createRange();
	...
	{
		bcr=topNode.getBoundingClientRect();
	}
	
	var scrollableElement;
	
	if(!(scrollTop=this.documentContainer.ownerDocument.documentElement.scrollTop))
	{
		scrollTop=this.documentContainer.ownerDocument.body.scrollTop;
		scrollableElement=this.documentContainer.ownerDocument.body;
	}
	else
	{
		scrollableElement=this.documentContainer.ownerDocument.documentElement;
	}
	
	if(bcr.top<0)
	{
		scrollableElement.scrollTop+=bcr.top-10;		
	}
	if(!bottomNode)
	{
		return;
	...
	return;
	if(bottomNode.isText())
	{
		var range=this.documentContainer.ownerDocument.createRange();
		
		range.selectNode(bottomNode);
		
		bcr=range.getBoundingClientRect();
	}
	else
	{
		bcr=bottomNode.getBoundingClientRect();
	}
	if(bcr.bottom>this.documentContainer.ownerDocument.documentElement.clientHeight)
	{
		scrollableElement.scrollTop+=bcr.bottom-this.documentContainer.ownerDocument.documentElement.clientHeight+10;
	}
	
}
function EditableContent_scrollToSelection()
{
	var ranges;
	
	this.documentContainer.focus();
	
	ranges=this.window.contentSelection.getRanges();
	this.scrollToNodes(ranges.ranges[0].startContainer, ranges.ranges[ranges.ranges.length-1].endContainer );
...
{
	var range, ranges, newContainer, containers=[], validRanges=[], i;
	
	ranges=this.window.contentSelection.getRanges();//this.getRanges();
	
	for(i=0; i<ranges.ranges.length; i++)
	{
		range=ranges.ranges[i];
		
		newContainer=this.surroundContentFragment(tagName, tagAttributes,
																	range.startIsChildNode ? 
																	range.startContainer.parentNode : range.startContainer, 
																	range.startIsChildNode ? 
																	range.startContainer.getIndex()+range.startShiftOffset : range.startOffset,
																	range.endIsChildNode ? 
																	range.endContainer.parentNode : range.endContainer, 
																	range.endIsChildNode ? range.endContainer.getIndex() +range.endShiftOffset : range.endOffset, 
																	true);
		...
		containers[containers.length]=newContainer;
		validRanges[validRanges.length]=range;
	}
	...
	for(i=0; i<containers.length; i++)
	{
		this.addSelection(containers[i][0], 0, 
			...
			validRanges[i].isCustom);
	}
	if(ranges.boundaryNodes)
	{
		var border;
		
		for(i=0; i<ranges.boundaryNodes.length; i++)
		{
			border=this.window.contentSelection.addBorder(ranges.boundaryNodes[i][0], ranges.boundaryNodes[i][1]);
			border.isSolid();
		}
	}
	return containers;
...
function EditableContent_surroundContentFragment(tagName, tagAttributes, node, offset, rangeEndContainer, rangeEndOffset, expand)
{
	...
	//console.log("args.inlineEdgeNodes.length= ", args.inlineEdgeNodes.length);
	for(i=0; i<args.inlineEdgeNodes.length; i++)
	{	
		//console.log(args.inlineEdgeNodes[i]);
...
function EditableContent_collectInsertionBoundaries(node, offset, close, endVisit, ca )
{
	...
	if(!ca.contentEditable)
	{
		ca.contentEditable=[];
		
		var ancestors, prevContentEditable=null;
		
		ancestors=node.getAncestors(!close);//!close: add the node itself to the list of ancestors
											//if the visit starts with an ending node then the last element of ca.contentEditable will be popped
		
		for(var i=0; i<ancestors.length; i++)
		{
			//console.log(ancestors[i]);
			if(ancestors[i]===this.documentContainer)
			{
				prevContentEditable=this.documentContainer.contentEditable;
			}
			...
			if(typeof(ancestors[i].contentEditable)=="undefined" || ancestors[i].contentEditable=="inherit")
			{
				ca.contentEditable.push(prevContentEditable);
			}
			else
			{
				ca.contentEditable.push(ancestors[i].contentEditable);
				prevContentEditable=ancestors[i].contentEditable;
			}
	...
	if(!close)
	{
		contentEditable=!node.isElement() || node.contentEditable=="inherit" ? ca.contentEditable[ca.contentEditable.length-1] : (node.contentEditable=="true"?true:false);
		ca.contentEditable.push(contentEditable);
	}
	else
	{
		contentEditable=ca.contentEditable.pop();
	}
...
function EditableContent_onKeyDown(evt)
{
	if(evt.keyCode==46 || evt.keyCode==8)//backspace or delete
	{
		if(this.window.contentSelection.isCollapsed())
		{
			return;
		}
		evt.preventDefault();
...
function EditableContent_preWrapTextNode(node, ca, close)
{
	if(!node.isText())
	{
		return;
	...
	parentStyle=node.parentNode.getComputedStyle();
	
	if(parentStyle.whiteSpace=="pre-wrap" || parentStyle.whiteSpace=="pre")
	{
		return;
	}
	...
	if(!ca.regexpStart)
	{
		...
		ca.regexpStart=/^[ \r\n\t]+/;
		ca.regexpEnd=/[ \r\n\t]+$/;
		ca.range=this.documentContainer.ownerDocument.createRange();
	}
	
	var pattern;
	
	
	if(parentStyle.whiteSpace=="pre-line")
	{
		pattern="[ \t]+";
	}
	else
	...
	if(node.isVisible())
	{
		...
		node.textContent=node.textContent.replace(new RegExp(pattern, "g"), " ");
		
		/* If the node starts or ends with white spaces 
		 * then check if these spaces are visible
		 */		
		if(new RegExp("^"+pattern).test(node.textContent))
		{
			ca.range.setStart(node, 0);
			ca.range.setEnd(node, 1);
			if(ca.range.getBoundingClientRect().width<=0)
			{
		...
		if(new RegExp(pattern+"$").test(node.textContent))
		{
			...
			ca.range.setStart(node, node.textContent.length-1);
			ca.range.setEnd(node, node.textContent.length);
			if(ca.range.getBoundingClientRect().width<=0)
			{
	...
	{	
		ca.invisibleNodes[ca.invisibleNodes.length]=node;
	}
	
}
function EditableContent_preWrap()
{
	var ca, comment;
	
	ca={"invisibleNodes": []};
	
	this.documentContainer.visitDescendants([this, "preWrapTextNode"], ca);
	
	
	for(var i=0; i<ca.invisibleNodes.length; i++)
	{
		comment=this.documentContainer.ownerDocument.createComment(ca.invisibleNodes[i].textContent);
		ca.invisibleNodes[i].parentNode.insertBefore(comment, ca.invisibleNodes[i]);
		ca.invisibleNodes[i].parentNode.removeChild(ca.invisibleNodes[i]);
	}
	
	this.documentContainer.style.whiteSpace="pre-wrap";
}