...
function EditableContent(iframe, innerHTML, serializedHistory, removedNodesContainerInnerHTML)//always an iframe, need edited doc CSS in their own space
{
...
function EditableContent_stickAdjacentTextNodes(node, offset, close, endVisit, ca /*callbackArguments*/)
{
	...
	if(ca.prev && ca.prev.parentNode==node.parentNode && ca.prev.isText() && node.isText() )
	{
...
function EditableContent_normalize()
{
	if(this.documentContainer.normalize)
	{
	...
	{
		var ca={};
		this.visitSelectedNodes([this, "stickAdjacentTextNodes"], 
							ca
							);
