...
with(EditableContent)
{
	...
	prototype.undoHighlightParagraphs=EditableContent_undoHighlightParagraphs;
	...
	prototype.getNotTransparentBackgroundDescendants=EditableContent_getNotTransparentBackgroundDescendants;
	prototype.getNodeParagraphFragment=EditableContent_getNodeParagraphFragment;
	prototype.getNodeParagraph=EditableContent_getNodeParagraph;
	prototype.highlightParagraphFragment=EditableContent_highlightParagraphFragment;
	prototype.highlightParagraph=EditableContent_highlightParagraph;
	prototype.unhighlightParagraphFragment=EditableContent_unhighlightParagraphFragment;
	prototype.getRangeBoundary=EditableContent_getRangeBoundary;
	prototype.restoreRange=EditableContent_restoreRange;
	//prototype.getRangeBoundaries2=EditableContent_getRangeBoundaries2;
	prototype.unhighlightParagraph=EditableContent_unhighlightParagraph;
	prototype.rangeIntersectsParagraph=EditableContent_rangeIntersectsParagraph;
	prototype.isRangeWithinParagraph=EditableContent_isRangeWithinParagraph;
	prototype.paragraphsEquals=EditableContent_paragraphsEquals;
	prototype.highlightCurrentParagraph=EditableContent_highlightCurrentParagraph;
	prototype.collectPargraphFragmentBoundaries=EditableContent_collectPargraphFragmentBoundaries;
	prototype.getNodeParagraphLeftBoundary=EditableContent_getNodeParagraphLeftBoundary;
...
function EditableContent_surroundInsertionBoundaryNodes(tagName, startNode, startOffset, endNode, endOffset, modifiedNodes)
{
	...
	else
	{
		...
		splitLeft=splitInfo=commonAncestorNode.splitAtDescendant(splitStart, splitStartOffset, true, false);
	...
	else
	{
		...
		splitRight=splitInfo=commonAncestorNode.splitAtDescendant(splitEnd, splitEndOffset, false, false);
		splitInfo=splitInfo[splitInfo.length-1];
	...
	*/
	...
	return { "container": containerElement, "splitLeft": splitLeft, "splitRight": splitRight};
}
...
EditableContent.prototype.getRangeBoundaries3=EditableContent_getRangeBoundaries3;
function EditableContent_getRangeBoundaries3(range, expand)
{	
	//var range=this.getRange();
...
EditableContent.prototype.getRanges=EditableContent_getRanges;
function EditableContent_getRanges()
{
	// @fixme : join adjacent ranges
	var range, ranges=[];
	
	if(this.window.getSelection().rangeCount<2)
	{
		return [this.getRange()];
	}
	//console.log("this.window.getSelection().rangeCount="+this.window.getSelection().rangeCount);
	for(var i=0; i<this.window.getSelection().rangeCount; i++)
	{
		ranges[ranges.length]=this.getRangeBoundaries3(this.window.getSelection().getRangeAt(i));
		//console.logNode(ranges[ranges.length-1].startContainer, "", "", "ranges[ranges.length]: ")
	}
	return ranges;
}
EditableContent.prototype.getSplittedRangeBoundaries2=EditableContent_getSplittedRangeBoundaries2;
function EditableContent_getSplittedRangeBoundaries2(range, splittedNodes, leftSide)
{
	/*
	 * SPLIT_LEFT_NODE=0;
			SPLIT_RIGHT_NODE=1;
			...
			SPLIT_OFFSET=4
	 */
	var splitLeftNode, splitRightNode, splitOffset, splitOriginalNode;
	for(var i=0; i<splittedNodes.length; i++)
	{
		console.logNode(range.startContainer, "", "", "range.startContainer: ")
		splitLeftNode=splittedNodes[i][SPLIT_LEFT_NODE];
		splitRightNode=splittedNodes[i][SPLIT_RIGHT_NODE];
		splitOffset=splittedNodes[i][SPLIT_OFFSET];
		splitOriginalNode=splittedNodes[i][splittedNodes[i][SPLIT_NODE_INDEX]];
		
		if(range.startContainer==splitOriginalNode)
		{
			...
			{
				console.log("splitOffset-range.startOffset="+(splitOffset+"-"+range.startOffset));
				range.startContainer=splitRightNode;
				range.startOffset=splitOffset-range.startOffset
			}
			console.log("Range start splitted: ");
		}
		if(range.endContainer==splitOriginalNode)
		{
			console.log("Range end splitted: ");
		}
		
		console.logNode(splitOriginalNode, "", "", "Original node: ");
		console.logNode(splitLeftNode, "", "", "Left node: ");
		console.logNode(splitRightNode, "", "", "Right node: ");
		console.log("Offset: "+splitOffset);
...
EditableContent.prototype.getSplittedRangeBoundaries=EditableContent_getSplittedRangeBoundaries;
function EditableContent_getSplittedRangeBoundaries(range, splitInfo)
{
	//check if current range start and/or end container are
	//within nodes splitted by one of previous operations
	for(var j=0; j<splitInfo.length; j++)
	{
		
		for( var k=0; k<splitInfo[j].containers.length; k++)
		{
			//range=this.getSplittedRangeBoundaries2(range, splitInfo[j].containers[k].splitLeft, true);
			//this.getSplittedRangeBoundaries2(range, splitInfo[j].containers[k].splitRight, false);
			//console.log(splitInfo[j].containers[k].splitLeft.length);
			//console.log(splitInfo[j].containers[k].splitRight.length);
...
EditableContent.prototype.surroundSelectedContentFragments=EditableContent_surroundSelectedContentFragments;
function EditableContent_surroundSelectedContentFragments(tagName, tagAttributes, select, expand)
{
	var ranges, splitInfo=[];
	
	ranges=this.getRanges();
	var selection;
		
	ranges=this.documentContainer.ownerDocument.defaultView.ContentSelection.prototype.___getRanges();
	this.clearSelection();
	for(var i=0; i<ranges.length; i++)
	{
		//console.logNode(ranges[i].startContainer, "", "", "AICI: ");
		//continue;
		if(i>0)
		{
			//ranges[i]=this.getSplittedRangeBoundaries(ranges[i], splitInfo);
		}
		splitInfo[splitInfo.length]=this.surroundContentFragment(tagName, tagAttributes, select,
				ranges[i].startContainer, ranges[i].startOffset,
				ranges[i].endContainer, ranges[i].endOffset, 
				expand);
		//console.log(splitInfo[splitInfo.length-1].containers.length);
	}
...
{
	var returnValue={"containers": [], "notAllowedAncestors": []};
	var args, containers;
	...
	{	
		sibn=this.surroundInsertionBoundaryNodes(tagName, args.inlineEdgeNodes[i].startNode, 
						args.inlineEdgeNodes[i].startOffset,
						args.inlineEdgeNodes[i].endNode, 
						args.inlineEdgeNodes[i].endOffset,
						splitInfo);
		returnValue.containers[returnValue.containers.length]=sibn;//.container;
		...
		{
			returnValue.containers[returnValue.containers.length-1].container[tagAttributeName]=tagAttributes[tagAttributeName];
		}
	}
	
	var notAllowedChildNodes, notAllowedAncestors;
	
	for(i=0; i<returnValue.containers.length; i++)
	{
		container=returnValue.containers[i].container;
		...
		for(j=0; j<notAllowedAncestors.length; j++)
		{
			...
			var splitInfo, splitLeft, splitRight;
			
			splitLeft=splitInfo=notAllowedAncestors[j].splitAtDescendant(container, 0, true, true);
			splitInfo=splitInfo[splitInfo.length-1];
			...
			splitRight=splitInfo=notAllowedAncestors[j].splitAtDescendant(container, container.childNodes.length, false, true);
			splitInfo=splitInfo[splitInfo.length-1];
			...
			returnValue.notAllowedAncestors[returnValue.notAllowedAncestors.length]={
				"splitLeft": splitLeft, "splitRight": splitRight	
			};
	...
	if(select && returnValue.containers.length>0)
	{
		
		this.addSelection(returnValue.containers[0].container, 0, returnValue.containers[returnValue.containers.length-1].container, returnValue.containers[returnValue.containers.length-1].container.childNodes.length);
	}
...
*/
EditableContent.prototype.getRangeBoundaries22=EditableContent_getRangeBoundaries22;
function EditableContent_getRangeBoundaries22(r)
{
	//var r/*range*/;
...
function EditableContent_unhighlightParagraph()
{
	...
	{
		rbs[rbs.length]=this.getRangeBoundaries22(rs[i]);
	}
...
function EditableContent_paragraphsEquals(par1, par2)
{
	...
	{
		if(par1[i].length!=par2[i].length)
		{
			return false;
		}
		for(var j=0; j<par1[i].length; j++)
		{
			if(par1[i][j]!=par2[i][j])
			{
				return false;
			}
		}
...
function EditableContent_highlightCurrentParagraph()
{
	...
	if(this.currentParagraph && this.paragraphsEquals(this.currentParagraph, par))
	{
		return;
	}
	
	var isRangeWithinParagraph=this.isRangeWithinParagraph(range, par);
	/*
