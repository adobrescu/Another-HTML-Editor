...
function EditableContent(iframe, innerHTML, serializedHistory)//always an iframe, need edited doc CSS in their own space
{
	...
	this.contentSelection=new ContentSelection(this.window);
...
{
	//@fixme find the right place where to call this.mutationHistory.setMirroringUndoneMutations
	this.mutationHistory.setMirroringUndoneMutations(true);
	
	var ranges, containers=[], i;
	
	ranges=this.contentSelection.getRanges();//this.getRanges();
...
{
	var i;
	var ranges=this.contentSelection.getRanges();//this.getRanges();
...
function EditableContent_unhighlightParagraph()
{
	...
	//r=this.getRange();
	rs=this.contentSelection.getRanges();//this.getRanges();
