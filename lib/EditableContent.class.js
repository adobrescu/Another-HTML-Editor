...
with(EditableContent)
{
	...
	prototype.getBreakableCommonAncestor=EditableContent_getBreakableCommonAncestor;
	prototype.surroundTextNodes=EditableContent_surroundTextNodes;
	prototype.collectSelectedContentEdges=EditableContent_collectSelectedContentEdges;
}
...
function EditableContent_setMutationHistoryRange(index)
{
	...
	//console.log("Set range "+index+" for "+mutationBatchIndex);
}
function EditableContent_restoreMutationHistoryRange(index)
{
	...
	//console.log("Restore range "+index+" for "+mutationBatchIndex);
...
function EditableContent_surroundNodes(tagName, startNode, startOffset, endNode, endOffset)
{
	...
	//console.logNode(splitStart, " ("+splitStartOffset+")", false, "splitStart: ");
	//console.logNode(splitEnd, " ("+splitEndOffset+")", false, "splitEnd: ");
	//console.logNode(commonAncestorNode, "", false, "Common ancestor: ");
...
function EditableContent_getBreakableCommonAncestor(startNode, endNode)
{
	//console.log("EditableContent_getBreakableCommonAncestor");
	//console.logNode(startNode, "", false, "Start: ");
	//console.logNode(endNode, "", false, "End: ");
	
	
	var startNodeAncestors, endNodeAncestors;
	
	startNodeAncestors=startNode.getAncestors();
	endNodeAncestors=endNode.getAncestors();
	
	for(var i=0; i<startNodeAncestors.length; i++)
	{
		if(startNodeAncestors[i]!=endNodeAncestors[i])
		{
			//common ancestor found
			//acum, pentru ambele noduri, se verifica daca nodurile parent pot fi 
			//sparte (nodurile parent dintre nodul testat si common ancestor-ul gasit)
			//daca unul dintre nodurile parent dintre ancestor si nodul testat nu poate fi spart,
			//atunci perechea startNode si endNode nu poate fi folosita pentru a sparge documentul
			for(var j=i; j<startNodeAncestors.length; j++)
			{
				
				if(!this.isTagBreakable(startNodeAncestors[j].tagName))
				{
					return null;
				}
				//console.logNode(startNodeAncestors[j], "", false, "start parent");
			}
			for(var j=i; j<endNodeAncestors.length; j++)
			{
				if(!this.isTagBreakable(endNodeAncestors[j].tagName))
				{
					return null;
				}
			}
			//console.logNode(endNodeAncestors[i-1]);
			return endNodeAncestors[i-1];
			//break;
		}
	}
...
function EditableContent_surroundTextNodes(tagName, inline, tagAttributes, select, node, offset, rangeEndContainer, rangeEndOffset)
{
	...
	{
		//console.log("not phrasing content x: "+tagName+", "+args.inlineEdgeNodes.length);
		//console.logNode(args.inlineEdgeNodes[0].endNode);
		j=0;
		var ind=[];
		/*
		for(i=1; i<args.inlineEdgeNodes.length; i++)
		{
			
			//compare an edge with next edges, trying to find first following edge 
			//that, if used as end node for splitting, won't split a not-breakable parent
			for(k=i; k<args.inlineEdgeNodes.length; k++)
			{
				var c=this.getBreakableCommonAncestor(args.inlineEdgeNodes[j].startNode,
												args.inlineEdgeNodes[k].endNode);
				//console.logNode(c, "", false, "Breakable common  ancestor");
				//console.log("---------------------------------------");
				if(!c)
				{
					ind[j]={ "startNode": args.inlineEdgeNodes[j].startNode, "startOffset": args.inlineEdgeNodes[j].startOffset, "endNode": args.inlineEdgeNodes[k-1].endNode,"endOffset": args.inlineEdgeNodes[k-1].endOffset};
					j=k;
					i=k;
				}
			}
		}
		ind[j]={ "startNode": args.inlineEdgeNodes[j].startNode, "startOffset": args.inlineEdgeNodes[j].startOffset, "endNode": args.inlineEdgeNodes[args.inlineEdgeNodes.length-1].endNode,"endOffset": args.inlineEdgeNodes[args.inlineEdgeNodes.length-1].endOffset};
		args.inlineEdgeNodes=ind;//[{ "startNode": args.inlineEdgeNodes[0].startNode, "startOffset": args.inlineEdgeNodes[0].startOffset, "endNode": args.inlineEdgeNodes[args.inlineEdgeNodes.length-1].endNode,"endOffset": args.inlineEdgeNodes[args.inlineEdgeNodes.length-1].endOffset}];
		*/
	}
	else
	{
		//console.log("phrasing content: "+tagName);
	}
	//this.mutationHistory.startMutationsBatch();
	for(i=0; i<args.inlineEdgeNodes.length; i++)
	{
		console.logNode(args.inlineEdgeNodes[i].startNode, " ("+args.inlineEdgeNodes[i].startOffset+")", false, "Start: ");
		console.logNode(args.inlineEdgeNodes[i].endNode, " ("+args.inlineEdgeNodes[i].endOffset+")", false, "End: ");
		console.log("-------------------------------------------------");
	...
	for(i=0; i<containers.length; i++)
	{
		...
		{
			//console.logNode(notAllowedChildNodes[j], "", false, "Not allowed descendant: ");
		//	try
			{
		...
		console.log(notAllowedAscendants.length);
		
		for(j=0; j<notAllowedAscendants.length; j++)
		{
			console.logNode(notAllowedAscendants[j], j+" ", false, "Not allowed ascendant");
			...
			if(!this.isTagBreakable(notAllowedAscendants[j].tagName))
			{
				continue;
			}
			
			//console.logNode(notAllowedAscendants[j], "", false, "Not allowed ascendant: ");
			...
			else
			{
				...
				notAllowedAscendants[j].splitChildAtDescendant(containers[i], containers[i].childNodes.length, false, true);
...
function EditableContent_getInlineNodesEdges(startNode, startOffset, endNode, endOffset, ca/*callback arguments*/, outerTagName, inline)
{
	...
	{
		console.log("Colelct transparent content");
	}
	else
	{
		
		ca.collectNotAllowedNodes=!Node.prototype.___tagIsPhrasingContent(outerTagName);
		this.visitSelectedNodes([this, "collectSelectedContentEdges"], 
						ca,
...
/**
 * 
 * EditableContent.collectSelectedContentEdges
 * 
 ...
 * @returns {undefined}
 */
function EditableContent_collectSelectedContentEdges(node, offset, close, endVisit, ca /*callbackArguments*/)
{
	...
	console.log(ca.collectNotAllowedNodes);
	//console.log((node.isText()?"Text":node.tagName)+" allowed in "+ca.outerTagName+": "+tagAllowed);
	...
	if(endVisit || (!tagAllowed && ca.collectNotAllowedNodes && ca.openNodes > 0) || !isBreakable)
	{
		if(!ca.startNewEdge)
		{
