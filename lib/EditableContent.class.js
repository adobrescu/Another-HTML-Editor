...
function EditableContent_getInlineNodesEdges(startNode, startOffset, endNode, endOffset, ca/*callback arguments*/, findTagNames)
{
	...
	ca.inlineEdgeNodes=[{"startNode": null, "startOffset": -1, "endNode": null, "endOffset":-1, "lastNode": null, "lastEndOffset": -1}];
	//ca.stack=[];
	ca.openNodes=0;
...
{	
	...
	if(endVisit)
	{
		...
		//console.logNode(ca.inlineEdgeNodes[numDelimiters-1].endNode, "("+ca.inlineEdgeNodes[numDelimiters-1].endOffset+"), end visit");
		return;
	}
	if(!this.nodeHasContentAtOffset(node, startOffset, !close))
	{
		return;
	}
	...
	if(close)
	{
		ca.inlineEdgeNodes[numDelimiters-1].lastNode=node;
		ca.inlineEdgeNodes[numDelimiters-1].lastOffset=endOffset;
		//console.log("Last used offset="+endOffset);
	}	
}