...
function EditableContent_collectPargraphFragmentBoundaries(node, offset, close, endVisit, ca )
{
	...
	else
	{
		...
		if(!close)
		{
			...
			//console.logNode(node, "", "", "Push  "+ca.stack.length+": ");
			//console.log("------------------------------------------------------------------");
		...
		else
		{
			if(ca.stack.length==1)
			{
				//ca.pfb[ca.pfb.length]=ca.stack[0];
				//console.log("end of a valid fragment");
			}
			...
			{
				ca.stack.pop();
				//console.logNode(node, "", "", "Pop  "+ca.stack.length+": ");
				if(ca.stack.length>0)
				{
					ca.stack[ca.stack.length-1].push(node);
				}
			}
			//console.logNode(node, "", "", "Append  "+ca.stack.length+": ");
...
function EditableContent_getNodeParagraph(node)
{
	...
	console.logNode(spcn, "", "", "Start visiting: ");
	...
	return ca.pfb;
	//console.log(ca.pfb);
	for(var i=0; i<ca.pfb.length; i++)
	{
		for(j=0; j<ca.pfb[i].length; j++)
		{
			//console.logNode(ca.pfb[i][j]);
		}
	}
	return;
	if(startPhrasingContentNode.isElement())
	{
		startPhrasingContentNode.style.backgroundColor="red";
	}
	else
	{
		startPhrasingContentNode.textContent="Test "+startPhrasingContentNode.textContent;
	}
	return;
	var pf/*paragraphFragments*/=[];
	var last=node;
	
	
	
	var siblingName="nextSibling";
	var childName="firstChild";
	
	var siblingNames=["nextSibling", "previousSibling"];
	var childNames=["firstChild", "lastChild"];
	var setLast=false;
	...
	for(var i=0; i<=1; i++)
	{
		...
		do
		{
			...
			if(i==0)
			{
				if(!last || !last.isPhrasingContent())
				{			
					break;
				}

				pf[pf.length]=this.getNodeParagraphFragment(last);

				last=pf[pf.length-1][pf[pf.length-1].length-1];
			}
			else
			{
				if(setLast)
				...
				else
				{
					...
					pf.splice(0, 0, this.getNodeParagraphFragment(last));
				}
			}
			
			if(last[siblingName])
			{
				last=last[siblingName];
			}
			else
			{
				while((last.parentNode.isPhrasingContent() || last.parentNode.hasTransparentContent()) && !last.parentNode[siblingName])
				{
					last=last.parentNode;
				}
				if(!last.parentNode.isPhrasingContent() && !last.parentNode.hasTransparentContent())
				{
					break;
				}
				last=last.parentNode[siblingName];	
			}

			if(last.hasTransparentContent())
			{
				while(last && last.hasTransparentContent())
				{
					last=last[childName];
				}
				continue;
			...
			{
				if(last.isPhrasingContent())
				{
					if(!last.isText())
					{

						if(!last[childName].isPhrasingContent())
						{
							break;
						}
						last=last[childName];
...
function EditableContent_highlightParagraphFragment(phrasingContentNodes)
{
	...
	for(var i=0; i<phrasingContentNodes.length; i++)
	{
		...
		{
			this.crtHighlightElements[this.crtHighlightElements.length-1].marker.className="block";
		}
...
function EditableContent_highlightParagraph(node)
{
	var i;
	...
	if(this.crtHighlightElements.length>0 && this.crtHighlightElements[0].marker==par[0][0])
	{
		return;
	}
...
function EditableContent_onSelectionChanged(evt)
{
	...
	////console.logNode(range.startContainer, " ("+range.startOffset+")", "", "AICI:");
	//console.logNode(range.endContainer, " ("+range.endOffset+")", "", "AICI:");
	this.highlightParagraph(startContainer);
}
