...
function EditableContent(iframe, innerHTML, serializedHistory, removedNodesContainerInnerHTML, serializedMutationHistoryRanges)//always an iframe, need edited doc CSS in their own space
{
	...
	this.restoreSerializedMutationHistoryRanges(serializedMutationHistoryRanges);
	this.mutationHistory=new MutationHistory(this.documentContainer, serializedHistory, removedNodesContainerInnerHTML);
...
with(EditableContent)
{
	...
	prototype.restoreSerializedMutationHistoryRanges=EditableContent_restoreSerializedMutationHistoryRanges;
}
...
function EditableContent_visitSelectedNodes(callback, callbackArguments, node, offset, endContainer, endOffset, visitSiblings, noCallback, depth)
{
	...
	if(callback && ( offset<closeOffset || ( node.isElement() && node.childNodes.length==0) ) )//!noCallback)
	{
		try
		{
...
function EditableContent_restoreSerializedMutationHistoryRanges(serializedMutationHistoryRanges)
{
	...
	for(var j=0; j<=1; j++) //MUTATION_HISTORY_RANGE_BEFORE=0 , MUTATION_HISTORY_RANGE_AFTER
	{
		for(var i=0; i<serializedMutationHistoryRanges[j].length; i++)
		{
			...
			console.log("-------------");
			console.logNode(startContainer, " ("+serializedMutationHistoryRanges[j][i].startOffset+")", false);
			console.logNode(endContainer, " ("+serializedMutationHistoryRanges[j][i].endOffset+")", false);
