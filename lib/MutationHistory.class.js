...
MutationHistory.prototype.joinMutations=MutationHistory_joinMutations;
function MutationHistory_joinMutations(i, j)
{
	console.log(i+", "+j +" / "+this.mutations.length);
	this.mutations[j].removedNodes=[];
	this.mutations[i].addedNodes=[];
	this.historyCursor=j;
	this.restoreCurrentMutation(true);
...
{	
	...
	console.log(mutationTypeToSkip);
	...
	for( ; this.historyCursor>=0 && this.mutations[this.history[this.historyCursor].mutation].type==mutationTypeToSkip; this.historyCursor--)
	{
		
		this.restoreCurrentMutation(!this.history[this.historyCursor].rtl);
		//this.historyCursor--;
	}
	if(slice && this.historyCursor<this.history.length-1)
	{
		console.log("Slice from: "+this.historyCursor+" / "+(this.history.length-1));
		console.log("Slice mutations: "+this.history[this.historyCursor+1].mutation+" / "+this.mutations.length);
		this.mutations.splice(this.history[this.historyCursor+1].mutation);
		this.history.splice(this.historyCursor+1);
		
		//console.log("Sliced from: "+this.historyCursor+" / "+(this.history.length-1));
		//console.log("Sliced mutations: "+this.history[this.historyCursor+1].mutation+" / "+this.mutations.length);
	}
	this.restoreCurrentMutation(!this.history[this.historyCursor].rtl);
	this.historyCursor--;
	return;
...
function MutationHistory_removeLastMutationBatchesByType(mutationBatchType)
{
	...
	for(var i=this.history.length-1; i>=0; i--)
	{
		...
		console.log(this.mutations[this.history[i].mutation].type);
		if(this.mutations[this.history[i].mutation].type!=mutationBatchType)
		{
			//console.log("break "+i);
			break;
		}
	...
	console.log("History: "+(i)+" / "+this.history.length);
	console.log("Mutation: "+this.history[i].mutation);
	
	this.mutations.splice(this.history[i].mutation);
	...
	console.log("After: "+this.historyCursor+" / "+this.history.length);
