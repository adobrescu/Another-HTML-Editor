...
MutationHistory.prototype.onDOMCharacterDataMutation=MutationHistory_onDOMCharacterDataMutation;
function MutationHistory_onDOMCharacterDataMutation(mutation)
{
	console.log(mutation.oldValue+"\n->\n"+mutation.newText);
	
	var isInsert, iMax;
	var string1, string2;
	var startOffset, endOffset, diff;
	
	
	//string1 must be shorter than string2
	if(mutation.oldValue.length<mutation.newText.length)
	{
		mutation.isInsert=true;
		string1=mutation.oldValue;
		string2=mutation.newText;
	}
	else
	{
		mutation.isInsert=false;
		string1=mutation.newText;
		string2=mutation.oldValue;
	...
	for(endOffset=string2.length-1; endOffset>=0; endOffset--)
	{
		if(string1[endOffset-diff]!=string2[endOffset])
		{
			break;
		}
	}
	//alert(startOffset+":"+endOffset);
	mutation.insertedText=string2.substr(startOffset, endOffset-startOffset+1);
	mutation.startOffset=startOffset;
	mutation.endOffset=endOffset+1;
...
function MutationHistory_onDOMMutations(mutations)
{
	...
	for(i=mutations.length-1; i>=0; i--)
	{
		...
		if(!mutations[i].newText)
		{
			mutations[i].newText=mutations[i].target.textContent;
		}
		for(j=i-1; j>=0; j--)
		{
	...
	for(i=0; i<mutations.length; i++)
	{
		...
		if(mutations[i].type=="characterData")
		{
			this.onDOMCharacterDataMutation(mutations[i]);
			mutations[i].newValue=mutations[i].target.textContent;
			console.log("isInsert: "+mutations[i].isInsert+", insertedText: \""+mutations[i].insertedText+"\", start offset: "+mutations[i].startOffset+", end offset:"+mutations[i].endOffset);
			console.log("-----------------------------------------");
		}
		else
		...
		this.mutationBatches[this.mutationsBatchIndex].mutations[i+numExistingMutations]=mutations[i];
	}
...
function MutationHistory_undoCharacterData(mutation)
{
	...
	{
		mutation.target.textContent=mutation.target.textContent.substr(0, mutation.startOffset)+mutation.target.textContent.substr(mutation.endOffset)
	}
	else
	{
...
function MutationHistory_serialize()
{
	...
	for(var i=0; i<this.mutationBatches.length; i++)
	{
		...
		{
			serializedHistory[i][j]=this.serializeMutation(this.mutationBatches[i].mutations[j]);
		}
	}
...
function MutationHistory_restoreSerializedHistory(serializedHistory, removedNodesContainerInnerHTML)
{
	...
	{
		this.mutationBatches[i]={ "mutations": [] };
		for(var j=0; j<serializedHistory[i].length; j++)
		{
			this.mutationBatches[i].mutations[j]=this.restoreSerializedMutation(serializedHistory[i].mutations[j]);
