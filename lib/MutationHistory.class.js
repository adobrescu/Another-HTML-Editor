...
function MutationHistory_onDOMMutations(mutations)
{
	for(var i=0; i<mutations.length; i++)
	{
		...
		this.mutations[this.mutations.length]=mutations[i];
	}
...
{	
	...
	{
		if(this.mutations[i].type=="attributes" && this.mutations[i].attributeName=="style")
		{
			for(var styleAttributeName in this.mutations[i].oldStyle)
			{
				console.log(styleAttributeName, this.mutations[i].oldStyle[styleAttributeName]);
			}
		}
	}
}
