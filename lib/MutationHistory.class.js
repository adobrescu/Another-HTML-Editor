...
function MutationHistory(target, serializedHistory, selectionChangeObserver, DOMChangeObserver)
{
	...
	//if(this.target.ownerDocument.onselectionchange || this.target.ownerDocument.onselectionchange===null)
	{
		...
		console.log("Check on selection change="+this.supportSelectionChangeEvent);
	...
	if(!this.supportSelectionChangeEvent)
	{
		this.pseudoOnSelectionChangeHandler2=new Function("evt", "MutationHistory.prototype.___instances["+MutationHistory.prototype.___numInstances+"].pseudoOnSelectionChange(evt)" );
		
		for(var i=0; i<MutationHistory.prototype.___pseudoOnSelectionChangeEvents.length; i++)
		{
			this.target.addEventListener(MutationHistory.prototype.___pseudoOnSelectionChangeEvents[i], this.pseudoOnSelectionChangeHandler2, false);
		}
		...
		this.pseudoOnSelectionChangeHandler=new Function("evt", "MutationHistory.prototype.___instances["+this.instanceNum+"].handlePseudoOnSelectionChange(evt);");
	}
...
with(MutationHistory)
{
	...
	prototype.___pseudoOnSelectionChangeEvents=["mouseup", "keypress", "keyup", "paste", "cut", "drop"];
	...
	prototype.handlePseudoOnSelectionChange=MutationHistory_handlePseudoOnSelectionChange;
	prototype.pseudoOnSelectionChangeHandler=null;
	prototype.pseudoOnSelectionChange=MutationHistory_pseudoOnSelectionChange;
...
function MutationHistory_release()
{
	for(var i=0; i<MutationHistory.prototype.___pseudoOnSelectionChangeEvents.length; i++)
	{
		this.target.removeEventListener(MutationHistory.prototype.___pseudoOnSelectionChangeEvents[i], this.pseudoOnSelectionChangeHandler2, false);
	}
...
MutationHistory.prototype.recordMutations=MutationHistory_recordMutations;
...
function MutationHistory_recordMutations(mutations)
{
	...
	var i, j;
	console.log("DOM "+this.DOMModifiedOutside);
...
function MutationHistory_handlePseudoOnSelectionChange(evt)
{
	...
	if(!this.lastRangeBeforeDOMChanges2 ||
		...
		same) )
	{	
		setTimeout(this.pseudoOnSelectionChangeHandler, 100, evt);// "MutationHistory.prototype.___instances["+this.instanceNum+"].handlePseudoOnSelectionChange();", 10);
...
function MutationHistory_pseudoOnSelectionChange(evt)
{	
	console.log("Changed: "+evt.type);	
if(evt.type=="drop" || evt.type=="paste")
	{
		...
		setTimeout(this.pseudoOnSelectionChangeHandler, 100, evt);//
		return;
	}
	
	this.handlePseudoOnSelectionChange(evt);	
}
function MutationHistory_onMouseDown(evt)
{
	this.onMouseDownRange2=this.getRanges();
}
function MutationHistory_onSelectionChange(evt, test)
{
	console.log("evt type="+evt.type+(evt.test?" test":""));
...
{
	console.log("MutationHistory_onDOMModifiedOutside: "+evt.type);
	this.DOMModifiedOutside=true;
}