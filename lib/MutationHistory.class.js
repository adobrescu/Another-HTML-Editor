...
function MutationHistory(target, serializedHistory, selectionChangeObserver, DOMChangeObserver)
{
	...
	this.numIDs=1;
	this.idPrefix="mhid_";
	this.lastRangeBeforeDOMChanges={"startContainer": null, "startOffset": -1, "endContainer": null, "endOffset": -1};
	...
	if(this.mirrorUndoneMutations && this.historyCursor<=this.history.length-1)
	{
		...
		if(historyStep>=this.historyCursor)//+1)
		{
			...
			for(i=historyStep; i>=this.historyCursor+1; i--)
			{
				...
				mirroredHistorySequence[mirroredHistorySequence.length]={ "mutation": this.history[i].mutation, "reversed": true, "rtl": true, "history_step": this.mirrorUndoneMutations};//this.reverseMutationBatch(this.history[i]);
				//this.history[this.history.length]={ "mutation": this.history[i].mutation, "reversed": true, "rtl": true, "history_step": this.mirrorUndoneMutations};//this.reverseMutationBatch(this.history[i]);
...
function MutationHistory_serialize()
{
	...
	{
		serializedHistory.mutations[i]={"records": [], "rangeBefore": this.mutations[i].rangeBefore, "rangeAfter": this.mutations[i].rangeAfter};
		
		if(serializedHistory.mutations[i].rangeBefore.startContainer)
		{
		//	serializedHistory.mutations[i].rangeBefore.startContainer=this.serializeMutationNode(serializedHistory.mutations[i].rangeBefore.startContainer);
		//	serializedHistory.mutations[i].rangeBefore.endContainer=this.serializeMutationNode(serializedHistory.mutations[i].rangeBefore.endContainer);
		//	serializedHistory.mutations[i].rangeAfter.startContainer=this.serializeMutationNode(serializedHistory.mutations[i].rangeAfter.startContainer);
		//	serializedHistory.mutations[i].rangeAfter.endContainer=this.serializeMutationNode(serializedHistory.mutations[i].rangeAfter.endContainer);
		}
...
function MutationHistory_restoreSerializedHistory(serializedHistory)
{
	...
	for(var i=0; i<serializedHistory.mutations.length; i++)
	{
		...
		this.mutations[i]={ "records": [], 
			"rangeBefore": serializedHistory.mutations[i].rangeBefore,  
			"rangeAfter": serializedHistory.mutations[i].rangeAfter };
		
		if(this.mutations[i].rangeBefore.startContainer)
		{
