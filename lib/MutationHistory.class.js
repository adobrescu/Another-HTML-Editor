...
function MutationHistory(target, serializedHistory, selectionChangeObserver, DOMChangeObserver)
{
	...
	{
		this.target.ownerDocument.defaultView.contentSelection.addSelectionChangeEventListener(this.onSelectionChangeHandler);
...
{
	var serializedHistory, rangeNames=["Before", "After"], rangeName;
	...
	for(var i=0; i<this.mutations.length; i++)
	{
		...
		for(var k=0; k<2; k++)
		{
			rangeName="ranges"+rangeNames[k];
			
			for(var j=0; j<serializedHistory.mutations[i][rangeName].ranges.length; j++)
			{
				if(serializedHistory.mutations[i][rangeName].ranges[j].startContainer)
				{
					serializedHistory.mutations[i][rangeName].ranges[j].startContainer=this.serializeMutationNode(serializedHistory.mutations[i][rangeName].ranges[j].startContainer);
					serializedHistory.mutations[i][rangeName].ranges[j].endContainer=this.serializeMutationNode(serializedHistory.mutations[i][rangeName].ranges[j].endContainer);
					
				}
			}
			for(var j=0; j<serializedHistory.mutations[i][rangeName].boundaryNodes.length; j++)
			{
				for(var l=0; l<serializedHistory.mutations[i][rangeName].boundaryNodes[j].length; l++)
				{
					serializedHistory.mutations[i][rangeName].boundaryNodes[j][l]=this.serializeMutationNode(serializedHistory.mutations[i][rangeName].boundaryNodes[j][l]);
...
function MutationHistory_restoreSerializedHistory(serializedHistory)
{
	...
	for(var i=0; i<serializedHistory.mutations.length; i++)
	{
		...
		for(var j=0; j<this.mutations[i].rangesBefore.ranges.length; j++)
		{
			if(this.mutations[i].rangesBefore.ranges[j].startContainer)
			{
				this.mutations[i].rangesBefore.ranges[j].startContainer=this.restoreSerializedMutationNode(this.mutations[i].rangesBefore.ranges[j].startContainer);
				this.mutations[i].rangesBefore.ranges[j].endContainer=this.restoreSerializedMutationNode(this.mutations[i].rangesBefore.ranges[j].endContainer);
				
				this.mutations[i].rangesAfter.ranges[j].startContainer=this.restoreSerializedMutationNode(this.mutations[i].rangesAfter.ranges[j].startContainer);
				this.mutations[i].rangesAfter.ranges[j].endContainer=this.restoreSerializedMutationNode(this.mutations[i].rangesAfter.ranges[j].endContainer);
			}
		}
...
function MutationHistory_getRanges()
{	
	return this.target.ownerDocument.defaultView.contentSelection.getRanges();
}
function MutationHistory_setRanges(ranges)
{
	var i, selectionLocked;
	
	this.target.ownerDocument.defaultView.contentSelection.lock(false);
		
	this.target.ownerDocument.defaultView.contentSelection.removeAllRanges();
	
	for(i=0; i<ranges.ranges.length; i++)
	{
		this.target.ownerDocument.defaultView.contentSelection.addRange(ranges.ranges[i]);
	}
	if(ranges.boundaryNodes)
	{
		for(i=0; i<ranges.boundaryNodes.length; i++)
		{
			this.target.ownerDocument.defaultView.contentSelection.isCustom=true;
			this.target.ownerDocument.defaultView.contentSelection.addBorder(ranges.boundaryNodes[i][0], ranges.boundaryNodes[i][1]);
		}
	...
	if(ranges.locked)
	{
		this.target.ownerDocument.defaultView.contentSelection.lock(true);
	}
...
function MutationHistory_onSelectionChange(evt)
{
	...
	this.lastRangeBeforeDOMChanges2=this.target.ownerDocument.defaultView.contentSelection.getRanges();
