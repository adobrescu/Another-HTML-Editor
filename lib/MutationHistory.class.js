...
function MutationHistory_onDOMMutations(mutations)
{
	...
	var i, j;
	//console.log(mutationsBatchIndex);
	this.mutationsBatchIndex++;
	
	if(this.mutations[this.mutationsBatchIndex])
	{
		var iMax=this.mutations.length-1;
		var lastRevertedIndex;
		
		lastRevertedIndex=this.mutationsBatchIndex;
		while(this.mutations[lastRevertedIndex].reverted)
		{
			console.log("Reverted: "+lastRevertedIndex);
			lastRevertedIndex++;
		}
		
		for(i=iMax; i>=lastRevertedIndex; i--)
		{
			if(this.mutations[i].reverted)
			{
				console.log("Already reverted");
				continue;
			}
			this.mutations[this.mutations.length]=this.reverseMutationBatch(this.mutations[i]);
			this.mutations[this.mutations.length-1].reverted=true;
			this.mutations[i].reverted=true;
		}
		this.mutationsBatchIndex=this.mutations.length;
	}
	//console.log("this.mutationsBatchIndex="+this.mutationsBatchIndex);
	//console.log("---------------------------------------------");
	...
	{
		this.mutations[this.mutationsBatchIndex]={"rangeBefore": this.lastRangeBeforeDOMChanges, 
	...
	var numExistingMutations=this.mutations[this.mutationsBatchIndex].records.length;
	...
	for(i=0; i<mutations.length; i++)
	{
		...
		this.mutations[this.mutationsBatchIndex].records[i+numExistingMutations]=mutation;//mutations[i];
	}
...
{
	for(var i=this.mutationsBatchIndex+1; i<this.mutations.length; i++)
	{
		this.redo();
	}
}
...
{	
	...
	console.log("UNDO "+(this.mutations[this.mutationsBatchIndex].reverted?" reverted":""));
	for(var i=this.mutations[this.mutationsBatchIndex].records.length-1; i>=0; i--)
	{
		
		switch(this.mutations[this.mutationsBatchIndex].records[i].type)
		{
			case "childList":
				this.undoChildList(this.mutations[this.mutationsBatchIndex].records[i]);
				break;
			case "attributes":
				this.undoAttributes(this.mutations[this.mutationsBatchIndex].records[i]);
				break;
			case "characterData":
				
				this.undoCharacterData(this.mutations[this.mutationsBatchIndex].records[i]);
				break;
		}		
	}
	this.setRange(this.mutations[this.mutationsBatchIndex].rangeBefore);
	...
	while(true)
	{
		...
		if(this.mutations[this.mutationsBatchIndex].type!=mutationTypeToSkip)
		{
			this.undoMutationsBatch(this.mutationsBatchIndex);
...
function MutationHistory_redo()
{
	...
	for(var i=0; i<this.mutations[this.mutationsBatchIndex].records.length; i++)
	{
		switch(this.mutations[this.mutationsBatchIndex].records[i].type)
		{
			case "childList":
				this.redoChildList(this.mutations[this.mutationsBatchIndex].records[i]);
				break;
			case "attributes":
				this.redoAttributes(this.mutations[this.mutationsBatchIndex].records[i]);
				break;
			case "characterData":
				this.redoCharacterData(this.mutations[this.mutationsBatchIndex].records[i]);
				break;
		}		
	}
	this.setRange(this.mutations[this.mutationsBatchIndex].rangeAfter);
	/* restart observing the target*/
	this.observe();
}
...
{
	console.log("UNDO CH: "+mutation.diff.offset+", \""+mutation.diff.text2+"\", \""+mutation.diff.text1+"\"");
...
function MutationHistory_removeLastMutationBatchesByType(mutationBatchType)
{
	...
	//console.log("this.mutationsBatchIndex="+this.mutationsBatchIndex+"/"+this.mutations.length);
	...
	//this.mutationsBatchIndex-=(this.mutations.length-i+1);
