console.logNode=function(node, endMsg, closeTag, startMsg)
{
	...
	this.log((typeof startMsg!="undefined" ?startMsg:"")+(node.nodeType==3?(closeTag?"":"> ")+"\""+node.textContent+"\""+(closeTag?" <":""):""+(node.nodeType==1?"<"+(closeTag?"/":"")+(node.tagName+(node.id?"#"+node.id:"")+">"):node.nodeType))+(endMsg?endMsg+" ":""));
}
...
console.logMutationRecord=function(rec, i)
{
	console.log(i+". "+rec.type);
		...
		{
			case "characterData":
				console.log("Change: \""+rec.oldValue+"\" -> \""+rec.target.textContent+"\"");
				break;
			case "attributes":
...
console.logObjectsDiff=function(obj1, obj2)
{
	for(var s in obj1)
	{
		if(typeof(obj1[s])=="function" || typeof(obj1[s])=="object" || obj1[s]==obj2[s])
		{
			continue;
		}
		console.log(s+": "+obj1[s]+" == "+ obj2[s]);
	}
}