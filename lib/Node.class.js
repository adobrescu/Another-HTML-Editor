...
with(Node)
{
	...
	prototype.isVisible=Node_isVisible;
}
...
function Node_splitAtDescendant(descendant, offset, newNodesToLeft, splitNode)
{
	...
	sel=window.contentSelection;//(this.ownerDocument.defaultView);
 ...
 * @returns {undefined}
 */
function Node_getAncestors(skipThis)
{
	var ancestors=[];
	for(var ancestor=skipThis?this.parentNode:this; ancestor; ancestor=ancestor.parentNode)
	{
...
function Node_isVisible()
{
	...
	else
	{
		...
		bcrs=range.getBoundingClientRect();
		
		console.log(this.textContent, bcrs.width);
