...
with(Node)
{
	...
	prototype.__html5Tags={
		...
		"BR": {"category": FLOW_CONTENT | PHRASING_CONTENT, "contentCategory": 0},
...
{
	return this.isElement() && (this.getContentCategory() & PHRASING_CONTENT);
}
...
function Node_splitChildAtDescendant(childNode, offset, toLeft, splitNode, dbg)
{
	var newNode;
	...
	if(dbg)
	{	
		//console.logNode(prevChildNode, "", false, "Node.splitChildAtDescendant: ");
	}
			
	return toLeft ? { "left": newNode, "right": prevChildNode} : { "left": prevChildNode, "right": newNode};
			
	return toLeft ? childNode : newNode;
