...
function ContentSelection(window)
{
	this.window=window;
...
function ContentSelection____takeSnapshot(window)
{	
...
{
	this.window.ContentSelection.prototype.___takeSnapshot(this.window);
}
ContentSelection.prototype.addRange=ContentSelection_addRange;
function ContentSelection_addRange(rangeBoundaries)
{
...
function ContentSelection_getRanges(noRefresh)
{
	...
	for(var i=0; i<this.window.ContentSelection.prototype.___ranges.length; i++)
	{
		ranges[i]=this.getRangeFromBoundaries(this.window.ContentSelection.prototype.___ranges[i]);
...
const TABLE_SECTIONING_CONTENT=2048;
const LI_CONTENT=4096;
...
with(Element)
{
	prototype.addClassName=Element_addClassName;
	prototype.removeClassName=Element_removeClassName;
	prototype.hasClassName=Element_hasClassName;
	prototype.getElementsByTagName=Element_getElementsByTagName;
}


function Element_addClassName(className)
{
        var re;
...
function Element_removeClassName(className)
{
	var re;
...
function Element_hasClassName(element, className)
{
        var re;
...
function Element_getElementsByTagName(tagName)
{
	var ca={};
	
	ca.tagName=tagName.toUpperCase();
	ca.elements=[];
	
	this.visitDescendants(function(node, ca)
							{
								if(node.isElement() && node.tagName==ca.tagName)
								{
									ca.elements[ca.elements.length]=node;
								}
							}, 
							ca/*callbackArguments*/, false, false);
	return ca.elements;
}
with(Node)
{
    ...
    After a dt element inside a dialog element.*/},
		...
		"X-FLOW-CONTENT-NOT-EDITABLE": {"category": FLOW_CONTENT, "contentCategory": ALL_CONTENT},
    ...
    If the element does not have a src attribute: one or more source elements, then, transparent.*/},
		...
		*/
		...
		//----Tabular data
		"TABLE": {"category": FLOW_CONTENT, "contentCategory": TR_CONTENT | TABLE_SECTIONING_CONTENT},
		"CAPTION": {"category": 0, "contentCategory": PHRASING_CONTENT},
		//"COLGROUP": {"category": [], "contentCategory": []},
		//"COL": {"category": [], "contentCategory": []},
		"TBODY": {"category": TABLE_SECTIONING_CONTENT, "contentCategory": TR_CONTENT},
		"THEAD": {"category": TABLE_SECTIONING_CONTENT, "contentCategory": TR_CONTENT},
		"TFOOT": {"category": TABLE_SECTIONING_CONTENT, "contentCategory": TR_CONTENT},
		"TR": {"category": TR_CONTENT, "contentCategory": TD_CONTENT},
		"TD": {"category": TD_CONTENT, "contentCategory": ALL_CONTENT},
		"TH": {"category": TD_CONTENT, "contentCategory": ALL_CONTENT},
	...
	prototype.___notBreakableTagNames=["OL", "UL", "LI", "TABLE", "TBODY", "THEAD", "TFOOT", "TH", "TR", "TD"];
	...
	prototype.isTagBreakable=Node_isTagBreakable;
	...
	prototype.removeChildren=Node_removeChildren;
	...
	prototype.getAncestorByTagName=Node_getAncestorByTagName;
...
function Node_isTagBreakable(tagName)
{
	for(var i=0; i<Node.prototype.___notBreakableTagNames.length; i++)
	{
		if(tagName==Node.prototype.___notBreakableTagNames[i])
		{
			return false;
		}
	}
...
function Node_cloneNode(cloneChildNodes, noObserver)
{
	var clone=this.mutationHistorycloneNode(cloneChildNodes);
	
	if(!noObserver)
	{
		window.parent.ed.currentEditableContent.mutationHistory.observe(clone);
	}
	return clone;
}
...
function Node_splitAtDescendant(descendant, offset, newNodesToLeft, splitNode)
{
	...
	var prevOffset;
	var ranges;
	var sel, selectionUpdated;
	
	sel=new ContentSelection(this.ownerDocument.defaultView);
	...
	for( ; descendant!=(splitNode?this.parentNode:this); descendant=descendant.parentNode)
	{
		...
		if(newNodesToLeft)
		{
			...
			SPLIT_OFFSET=4
			*/
			splittedNodes[splittedNodes.length]=[newNode, descendant, 1, 0, prevOffset];
		}
		else
		{
			splittedNodes[splittedNodes.length]=[descendant, newNode, 0, 1, prevOffset];
		}
		...
		selectionUpdated=sel.updateSplittedRanges(descendant, newNode, newNodesToLeft, prevOffset);
	}
...
function Node_getAncestorByTagName(tagName)
{
	var ancestor;
