...
function ContentSelection_updateSplittedRanges(rangeContainer, newContainer, newContainerToLeft, splitOffset)
{
	...
	//console.log("ContentSelection.prototype.___ranges.length="+ContentSelection.prototype.___ranges.length);
	for(var i=0; i<ContentSelection.prototype.___ranges.length; i++)
	{
		...
		if(range.startContainer==rangeContainer)
		{
			...
			{
				//console.log("Split at or before range start: "+splitOffset+" / "+range.startOffset);
				range.startContainer=rightContainer;
				range.startOffset=range.startOffset-splitOffset;
			}
			else
			{
				//console.log("Split after range start: "+splitOffset+" / "+range.startOffset);
		...
		if(range.endContainer==rangeContainer)
		{
			...
			{
				//console.log("Split after range end: "+splitOffset+" / "+range.endOffset);
				range.endContainer=leftContainer;
			}
			else
			{
				//console.log("Split at or before range end: "+splitOffset+" / "+range.endOffset);
				range.endContainer=rightContainer;
				range.endOffset=range.endOffset-splitOffset;
			}
		}

		//this.selection.removeRange(range.range);
		//var newRange=this.window.document.createRange();
		//range.range.setStart(range.startContainer, range.startOffset);
		//range.range.setEnd(range.endContainer, range.endOffset);
		//console.logNode(range.startContainer, "", "", "range.startContainer: ("+range.startOffset+")");
		//console.logNode(range.endContainer, "", "", "range.endContainer: ("+range.endOffset+")");
