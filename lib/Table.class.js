...
function HTMLTableElement_buildCellsSelection(td1, td2)
{
	...
	{
		this.ownerDocument.defaultView.contentSelection.addBorder(this.selectedCells.sections[i][2], this.selectedCells.sections[i][3]);	
	}
}
...
{
	var sectionFirstCell, sectionLastCell, previousTD;
	
	this.anchorSelectedCell=null;
	
	for(var i=0; i<this.selectedCells.sections.length; i++)
	{
		previousTD=sectionFirstCell=this.selectedCells.sections[i][2];
		...
		for(var j=this.selectedCells.sections[i][0]+1; j<=this.selectedCells.sections[i][1]; j++)
		{
			...
			previousTD=this.selectedCells.cells[j];
		}
...
function HTMLTableElement_splitSelectedCells()
{
	...
	this.buildCellsSelection(startTD, this.logicalMatrix[endLogicalRowIndex][endLogicalCellIndex].cell);
