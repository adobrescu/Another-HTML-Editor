...
function HTMLTableElement_onClickCell(evt)
{
	...
	this.selectedCells[0]=this.getCellsBetween(this.anchorSelectedCell, td);
	...
	{
		section=sections[this.selectedCells[0][i].sectionIndex];
		var border=new Border(this.ownerDocument);
		
		border.surroundElements(section.rows[this.selectedCells[0][i].startRow].cells[this.selectedCells[0][i].startCol],
								section.rows[this.selectedCells[0][i].endRow].cells[this.selectedCells[0][i].endCol]);
		border.show();
	}
	return;
	var section;
	for(var i=0; i<this.selectedCells[0].length; i++)
	{
		...
		{
			for(var k=0; k<section.rows[j].cells.length; k++)
			{
				if(j>=this.selectedCells[0][i].startRow &&
					k>=this.selectedCells[0][i].startCol &&
					j<=this.selectedCells[0][i].endRow &&
					k<=this.selectedCells[0][i].endCol )
				{
					...
					if(k==this.selectedCells[0][i].startCol)
					{
						className+=" left";
					}
					if(j==this.selectedCells[0][i].endRow)
					{
						className+=" bottom";
					}
					if(k==this.selectedCells[0][i].endCol)
					{
						className+=" right";
					}
					
					section.rows[j].cells[k].addClassName(className);
					
				}
				else
				{
					section.rows[j].cells[k].removeClassName("selected");
				}
			}
		}
	}
	//td.addClassName("selected", td.hasClassName("selected"));
}
...
function HTMLTableElement_mergeCellsInBetweenCells(startTD, endTD)
{
	...
	{
		mergeTDs[0].appendChild(this.ownerDocument.createTextNode(" "));
	}
	else
	{
		mergeTDs[0].appendChild(this.ownerDocument.createElement("BR"));
	}
	var appendBR;
