...
function HTMLTableElement_onMouseDown(evt)
{
	...
	return;
	var border=HTMLTableElement.prototype.___borders[HTMLTableElement.prototype.___borders.length]=new Border(this.ownerDocument);
	this.currentBorders=[border];
	border.surroundElements(this.anchorSelectedCell, this.anchorSelectedCell);
	border.show();
...
function HTMLTableElement_onMouseUp(evt)
{
	...
	for(var i=0; i<this.selectedCells.cells.length; i++)
	{
		...
		this.ownerDocument.defaultView.contentSelection.addRange(
				{ "startContainer": this.selectedCells.cells[i], "startOffset": 0, 
				"endContainer": this.selectedCells.cells[i], "endOffset": this.selectedCells.cells[i].childNodes.length } , true);
	}
...
HTMLTableElement.prototype.insertRow=HTMLTableElement_insertRow;
function HTMLTableElement_insertRow(logicalRowIndex)
{
	...
	{
		this.logicalMatrix[logicalRowIndex][0].cell.parentNode.parentNode.insertBefore(tr, this.logicalMatrix[logicalRowIndex][0].cell.parentNode);
	}
	else
	{
		sections=this.getSections();
	...
	for(var i=0; i<this.logicalMatrix[logicalRowIndex].length; i++)
	{
		if(logicalRowIndex>0 && this.logicalMatrix[logicalRowIndex][i].cell==this.logicalMatrix[logicalRowIndex-1][i].cell)
		{
			this.logicalMatrix[logicalRowIndex][i].cell.rowSpan++;
			this.logicalMatrix[logicalRowIndex][i].cell.style.backgrondColor="red";
			
			for(var j=0; j<this.logicalMatrix[logicalRowIndex][i].cell.colSpan-1; j++)
			{
				i++;
			}
		}
		...
		{
			td=this.ownerDocument.createElement(this.logicalMatrix[logicalRowIndex][i].cell.tagName);
